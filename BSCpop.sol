...
contract ERC721 is ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    constructor (string memory name_, string memory symbol_) public {
        ...
        _setBaseURI("https://metadata.bscpop.com/.netlify/functions/server/");
        //curve = msg.sender;
        
        palette.push("#f3ba2f"); //y: 1/5 chance (0.1992)
        palette.push("#FE0879"); //blue: 1/5 chance
        palette.push("#FF82E2"); //red: 1/5 chance
        palette.push("#0037B3"); //w: 1/5 chance
        palette.push("#70BAFF"); //black: 1/5 chance
        palette.push("#edffb1"); //green: rare 1/256 chance for a til
        // .. //
    }
    ...
    function mint(address to) public virtual returns (uint256 newTokenId) {
        require(msg.sender == curve, "BSCPOP: Minter is not the curve");
...
string memory svg = string(abi.encodePacked(
            "<svg width='300' height='300' style='background-color:",palette[toUint8(bhash,5)/51],"'>", 
            string(abi.encodePacked("<polygon fill='",palette[toUint8(bhash,0)/51],"' points='38.171 53.203 62.759 28.616 87.36 53.216 101.667 38.909 62.759 0 23.864 38.896 '/>")), // tile 1
            string(abi.encodePacked("<rect x='3.644' y='53.188' transform='matrix(0.7071 0.7071 -0.7071 0.7071 48.7933 8.8106)' fill='",palette[toUint8(bhash,8)/51],"' width='20.233' height='20.234'/>")), // tile 2
            string(abi.encodePacked("<polygon fill='",palette[toUint8(bhash,1)/51],"' points='38.171 73.408 62.759 97.995 87.359 73.396 101.674 87.695 101.667 87.703 62.759 126.611 23.863 87.716 23.843 87.696 '/>")), // tile 3
            string(abi.encodePacked("<rect x='101.64' y='53.189' transform='matrix(-0.7071 0.7071 -0.7071 -0.7071 235.5457 29.0503)' fill='",palette[toUint8(bhash,3)/51],"' width='20.234' height='20.233'/>")), // tile 4
            string(abi.encodePacked("<polygon fill='",palette[toUint8(bhash,2)/51],"' points='77.271 63.298 77.277 63.298 62.759 48.78 52.03 59.509 52.029 59.509 50.797 60.742 48.254 63.285 48.254 63.285 48.234 63.305 48.254 63.326 62.759 77.831 77.277 63.313 77.284 63.305 '/>")), // tile 5
            "</svg>"
        ));
    ...
    function burn(address burner, uint256 tokenId) public virtual {
        require(msg.sender == curve, "BSCPOP: Burner is not the curve"); // only curve can burn it
        require(burner == ownerOf(tokenId), "BSCPOP: Not the correct owner");
...
contract Curve {
    ...
    */
    constructor (address payable _creator, address _bscpop) {
        // create ERC721 contract with parameters (name, symbol).
        creator = _creator;
        reserve = 0;
        bscpop = ERC721(_bscpop); // todo: check naming availability
    }
    ...
    function mint() public virtual payable returns (uint256 _tokenId) {
        ...
        uint256 tokenId = bscpop.mint(msg.sender);
    ...
    function burn(uint256 tokenId) public virtual {
        ...
        bscpop.burn(msg.sender, tokenId);
